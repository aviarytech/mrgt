...
export class Glossary {
    ...
    getSafMap(safURL) {
        ...
        try {
            ...
            saf = yaml.load(fs.readFileSync(safURL, 'utf8'));
            const requiredProperties = ['scopetag', 'scopedir', 'curatedir', 'defaultvsn'];
     ...
     */
    async getMrgMap(mrgURL) {
        let mrg = {};
        try {
            // Try to load the MRG map from the `mrgURL`
            const mrgfile = fs.readFileSync(mrgURL, 'utf8');
            mrg = yaml.load(mrgfile);
            const requiredProperties = ['scopetag', 'scopedir', 'curatedir', 'vsntag'];
     ...
     * @returns A promise that resolves to the populated runtime glossary.
     */
    async populateRuntime(mrg, filename) {
        try {
            ...
            for (const entry of mrgEntries) {
                ...
                const modifiedAlternatives = new Set();
 ...
 */
function applyMacroReplacements(input, regexMap) {
